...
export default function CreateEvenement() {
  ...
  const [intervenantsDisponibles, setIntervenantsDisponibles] = useState<
    Intervenant[]
  >([]);
  const [intervenantsSelectionnes, setIntervenantsSelectionnes] = useState<
    number[]
  >([]);
  ...
  const toggleIntervenant = (id: number) => {
    if (intervenantsSelectionnes.includes(id)) {
      setIntervenantsSelectionnes(
        intervenantsSelectionnes.filter((i) => i !== id),
      );
    } else {
  ...
  const validerFormulaire = (): boolean => {
    ...
    if (mode === "presentiel" && !lieu.trim()) {
      showAlert(
        "Erreur",
        "Le lieu est obligatoire pour un événement présentiel",
      );
      return false;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const evenementData = {
        ...
        intervenants: intervenantsSelectionnes.map((id) => ({
          id,
        })) as Intervenant[],
      };
          ...
          <View style={styles.radioGroup}>
            ...
            >
              <View
                style={[
                  styles.radio,
                  mode === "presentiel" && styles.radioSelected,
                ]}
              >
            ...
            >
              <View
                style={[
                  styles.radio,
                  mode === "enligne" && styles.radioSelected,
                ]}
              >
              ...
              {intervenantsDisponibles.map((intervenant) => (
                ...
                >
                  <View style={styles.intervenantInfo}>
                    ...
                    <Text style={styles.intervenantSexe}>
                      {intervenant.sexe}
                    </Text>
                  </View>
                  {intervenantsSelectionnes.includes(intervenant.id!) && (
